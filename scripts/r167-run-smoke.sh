#!/bin/bash
# R167 dimni (vzorec r166 — ni novih rut, čista client + UI sweep): svež proces, repo v roksal-repo (prilagoditev r160 vzorca).
...
setsid node .next/standalone/server.js > /tmp/r167-server-smoke.log 2>&1 < /dev/null &
sleep 4
